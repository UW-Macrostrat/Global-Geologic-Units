...
Version info: 
Format: TSV
Information: Fonttype/Formatting recognition via a custom script. Utilizes the output of
    the Cuneiform OCR process.
    Column structure:
...
Format: TSV
Information: 
    NLP and Fonttype outputs (input.text and fonts.tex) combined into one DeepDive-ready table.
    Column structure:
...
Format: TSV
Information:
    "New" NLP (NLPCore 3.5.2). Same as the XML output above, but formatted into a DeepDive-ready table
    Column structure:
        docid (text) -- document's unique ID within our internal database
        sentid (integer) -- sentence's index within the document
        wordidx (integer[]) -- Word's index within the sentences
...
Format: TSV
Information:
    "New" NLP (NLPCore 3.5.2). Same as the XML output above, but formatted into a DeepDive-ready table. This output should match the format output by the "bazaar" utility (https://github.com/HazyResearch/bazaar)
    Column structure:
        docid (text) -- document's unique ID within our internal database
        sentid (integer) -- sentence's index within the document
        sentence (text) -- 
